...
if __name__ == "__main__":
	...
	feature_user_prod = sc.textFile('competition_tianchi/feature/'+target+'/user_prod', 1).map(lambda x : (x.strip().split("\t")[0]+"\t"+x.strip().split("\t")[2],x.strip()))
	feature_user_ict = sc.textFile('competition_tianchi/feature/'+target+'/user_ict', 1).map(lambda x : (x.strip().split("\t")[0]+"\t"+x.strip().split("\t")[1],"\t".join(x.strip().split("\t")[2:])))
	feature_prod = sc.textFile('competition_tianchi/feature/'+target+'/prod', 1).map(lambda x : (x.strip().split("\t")[0],"\t".join(x.strip().split("\t")[1:])))
	feature_user = sc.textFile('competition_tianchi/feature/'+target+'/user', 1).map(lambda x : (x.strip().split("\t")[0],"\t".join(x.strip().split("\t")[1:])))
